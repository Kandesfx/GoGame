...
const MainWindow = ({ onLogout, onBackToHome, initialMatch = null }) => {
  ...
  useEffect(() => {
    ...
    if (!currentMatch || (currentMatch.ai_level !== null && currentMatch.ai_level !== undefined)) {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (matchData && (matchData.result || matchData.finished_at)) {
          console.log('🏁 Match finished detected in polling:', { result: matchData.result, finished_at: matchData.finished_at })
        ...
        if (matchData && matchData.white_player_id && !currentMatch.white_player_id) {
  ...
  }, [currentMatch, gameOver, isProcessing, boardState.stones, boardState.currentPlayer])
  ...
  useEffect(() => {
    ...
    if (boardState.blackTimeRemaining === null && boardState.whiteTimeRemaining === null) {
      return
    }
    ...
    const timerInterval = setInterval(() => {
      setBoardState(prev => {
        const now = Date.now()
        // Tính thời gian đã trôi qua từ lần cập nhật cuối
        // Giả sử mỗi giây trừ 1 giây (đơn giản hóa)
        let newBlackTime = prev.blackTimeRemaining
        let newWhiteTime = prev.whiteTimeRemaining
        
        // Chỉ đếm ngược cho người chơi hiện tại
        if (prev.currentPlayer === 'B' && newBlackTime !== null && newBlackTime > 0) {
          newBlackTime = Math.max(0, newBlackTime - 1)
        } else if (prev.currentPlayer === 'W' && newWhiteTime !== null && newWhiteTime > 0) {
  ...
  const loadMatchState = async (matchId) => {
    try {
      ...
      const isAiMatch = response.data && response.data.ai_level !== null && response.data.ai_level !== undefined
      ...
      if (response.data && response.data.state) {
        ...
        let stones = {}
        if (state.board_position && typeof state.board_position === 'object' && !Array.isArray(state.board_position)) {
        ...
        console.log('🎮 Backend state:', { current_player: state.current_player, moves_count: state.moves?.length, board_position: stones })
        ...
        if (state.moves && state.moves.length > 0) {
          ...
          if (lastMoveData && Array.isArray(lastMoveData) && lastMoveData.length === 2) {
        ...
        if (state.moves && Array.isArray(state.moves)) {
          setMoveHistory(prev => {
            ...
            if (state.moves.length > prev.length) {
              ...
              const newMoves = state.moves.slice(prev.length).map((move, relativeIndex) => {
                ...
                if (Array.isArray(move) && move.length === 2) {
                  ...
                  return {
                    number: index + 1,
                    color: isAiMatch ? correctColor : (index % 2 === 0 ? 'B' : 'W'),
                    position: move,
                    captured: []
                  }
                  ...
                  if (move.position !== null && move.position !== undefined) {
                    ...
                    } else if (move.position.x !== undefined && move.position.y !== undefined) {
                  ...
                  return {
                    ...
                    color: isAiMatch ? correctColor : (move.color || (index % 2 === 0 ? 'B' : 'W')),
                    position: position,
              ...
              console.log('📝 Adding new moves from backend:', newMoves.length, 'moves (prev:', prev.length, ', backend total:', state.moves.length, ')')
              ...
              console.log('📝 Keeping current move history (backend has', state.moves.length, 'moves, current has', prev.length, ')')
              return prev
            }
          })
        }
        ...
        setBoardState(prev => {
          ...
          if (prevStonesKeys.length !== newStonesKeys.length || 
          ...
          let correctCurrentPlayer = state.current_player || state.to_move || prev.currentPlayer || 'B'
          if (isAiMatch) {
            ...
            if (correctCurrentPlayer !== 'B' && correctCurrentPlayer !== 'W') {
              console.warn(`⚠️ Invalid currentPlayer from backend: ${correctCurrentPlayer}, using previous: ${prev.currentPlayer}`)
          ...
          return {
            ...
            prisonersBlack: state.prisoners_black !== undefined ? state.prisoners_black : prev.prisonersBlack,
            prisonersWhite: state.prisoners_white !== undefined ? state.prisoners_white : prev.prisonersWhite,
        ...
        if (response.data) {
          setCurrentMatch(prev => {
            ...
            if (prev && (
              ...
              prev.black_player_username !== response.data.black_player_username ||
              prev.white_player_username !== response.data.white_player_username
            )) {
        ...
        if (response.data && (response.data.result || response.data.finished_at)) {
        ...
        console.log('✅ Loaded match state:', { stones, lastMove, boardSize: state.size, prisoners: { black: state.prisoners_black, white: state.prisoners_white }, gameOver: !!response.data?.result })
      } else {
  ...
  const handleCreateMatch = async (matchType, level, boardSize, playerColor = 'black') => {
    console.log('🎮 handleCreateMatch called with:', { matchType, level, boardSize, playerColor })
    try {
      ...
      if (matchType === 'ai') {
        ...
        const requestBody = { level, board_size: boardSize, player_color: playerColor }
        console.log('🎨 Creating AI match with request body:', JSON.stringify(requestBody))
  ...
  const handleBoardClick = async (x, y) => {
    if (!currentMatch || isProcessing || gameOver) {
      console.log('⚠️ Ignoring click - no match, already processing, or game over')
      return
    }
    ...
    try {
      ...
      if (matchCheckResponse.data?.result || matchCheckResponse.data?.finished_at) {
    ...
    try {
      ...
      console.log('📋 Response details:', {
        ...
        has_prisoners: 'prisoners_black' in response.data && 'prisoners_white' in response.data
      })
      ...
      setBoardState(prev => {
        try {
          ...
          if (moveData.board_diff && moveData.board_diff.removed && Array.isArray(moveData.board_diff.removed)) {
          ...
          if (moveData.board_diff && moveData.board_diff.added && typeof moveData.board_diff.added === 'object') {
            ...
            Object.keys(moveData.board_diff.added).forEach(key => {
              ...
              newStones[key] = color
              console.log(`➕ Added user stone at ${key}: ${color} (forced, ignoring backend color)`)
            })
          } else {
          ...
          const updatedState = {
            ...prev,
            stones: newStones,
            prisonersBlack: moveData.prisoners_black !== undefined ? moveData.prisoners_black : prev.prisonersBlack,
            prisonersWhite: moveData.prisoners_white !== undefined ? moveData.prisoners_white : prev.prisonersWhite,
      ...
      setMoveHistory(prev => {
        ...
        console.log('📝 Adding user move to history:', newMove, 'from currentPlayer:', color)
        return [...prev, newMove]
      })
      ...
      if (moveData.ai_move) {
        ...
        setBoardState(prev => {
          ...
          if (aiMove.board_diff && aiMove.board_diff.removed && Array.isArray(aiMove.board_diff.removed)) {
          ...
          if (aiMove.board_diff && aiMove.board_diff.added && typeof aiMove.board_diff.added === 'object') {
          ...
          } else if (aiMove.x !== undefined && aiMove.y !== undefined && aiMove.x !== null && aiMove.y !== null) {
          ...
          return {
            ...prev,
            stones: newStones,
            prisonersBlack: aiMove.prisoners_black !== undefined ? aiMove.prisoners_black : prev.prisonersBlack,
            prisonersWhite: aiMove.prisoners_white !== undefined ? aiMove.prisoners_white : prev.prisonersWhite,
            currentPlayer: userColor, // Sau AI move, đến lượt user
            lastMove: aiMove.x !== undefined ? { x: aiMove.x, y: aiMove.y } : prev.lastMove
          }
        })
        ...
        setMoveHistory(prev => {
          ...
          if (aiMove.is_pass === true || (aiMove.x === null && aiMove.y === null)) {
            // AI passed
            const newMove = {
              number: moveNumber,
          ...
          if (aiMove.position !== null && aiMove.position !== undefined) {
            ...
            if (Array.isArray(aiMove.position) && aiMove.position.length === 2) {
              position = aiMove.position
            } else if (aiMove.position.x !== undefined && aiMove.position.y !== undefined) {
              position = [aiMove.position.x, aiMove.position.y]
            }
          } else if (aiMove.x !== null && aiMove.x !== undefined && aiMove.y !== null && aiMove.y !== undefined) {
        ...
        if (aiMove.game_over || moveData.game_over) {
          console.log('🏁 Game over detected after AI move:', { aiMoveGameOver: aiMove.game_over, moveDataGameOver: moveData.game_over })
          // Reload match để lấy result đầy đủ
          try {
            const matchResponse = await api.get(`/matches/${currentMatch.id}`)
            if (matchResponse.data) {
      ...
      console.log('✅ User move completed, skipping reload to avoid color overwrite')
  ...
  const handlePass = async () => {
    ...
    try {
      ...
      if (passResponse.data && passResponse.data.ai_move) {
        ...
        setMoveHistory(prev => {
          ...
          if (position !== null) {
            ...
            console.log('📝 Adding AI pass to history (after pass, fallback):', newMove)
            return [...prev, newMove]
          }
        })
        ...
        if (isGameOver) {
          console.log('🏁 Game over detected after AI move:', { aiMoveGameOver: aiMove.game_over, passResponseGameOver: passResponse.data?.game_over })
          // Reload match để lấy result đầy đủ
          try {
            const matchResponse = await api.get(`/matches/${currentMatch.id}`)
            if (matchResponse.data) {
        ...
        const newCurrentPlayer = passResponse.data?.current_player || 
        ...
        if (currentMatch.ai_level) {
          setBoardState(prev => {
            ...
            const expectedPlayer = newCurrentPlayer === 'W' ? 'B' : newCurrentPlayer
            if (prev.currentPlayer !== expectedPlayer) {
              console.log(`🔧 Fixing currentPlayer after reload: ${prev.currentPlayer} -> ${expectedPlayer}`)
      ...
      if (error.code === 'ECONNABORTED') {
        ...
        alert('Failed to pass: ' + (error.response?.data?.detail || error.message))
      }
    } finally {
      setIsProcessing(false)
    }
  ...
  const loadFinalElo = async (matchData) => {
    if (!matchData || matchData.user_elo_change === null || matchData.user_elo_change === undefined) {
      return
    }
  ...
  const handleCancelMatch = async () => {
    ...
    if (currentMatch.white_player_id) {
      alert('Không thể hủy trận đấu đã có đủ người chơi. Vui lòng sử dụng chức năng đầu hàng.')
      return
    }
    
    if (!confirm('Bạn có chắc chắn muốn hủy bàn này? Người chơi khác sẽ không thể tham gia nữa.')) return
    ...
    try {
      ...
      alert('Không thể hủy bàn: ' + (error.response?.data?.detail || error.message))
    }
  }
  ...
  const handleResign = async () => {
    ...
    try {
      ...
      alert('Không thể đầu hàng: ' + (error.response?.data?.detail || error.message))
    }
  }
  ...
  const handleUndo = async () => {
    ...
    const isAIMatch = currentMatch.ai_level !== null && currentMatch.ai_level !== undefined
    ...
    try {
      ...
      if (result.undone_moves && result.undone_moves.length > 1) {
        console.log(`✅ Undo thành công: Đã hoàn tác ${result.undone_moves.length} nước đi (AI + User)`)
      } else {
        console.log('✅ Undo thành công:', result.undone_move || result.undone_moves?.[0])
      }
    } catch (error) {
      const errorMsg = error.response?.data?.detail || error.message || 'Không thể hoàn tác'
    ...
    <div className="main-window">
      <header className="main-header">
        ...
        <div className="header-actions">
          ...
          <button onClick={() => setShowMatchDialog(true)} className="btn btn-primary" title="Tạo trận đấu mới">
            <span>Trận mới</span>
          </button>
          <button 
          ...
          {currentMatch && currentMatch.room_code && (
            <div className="room-code-display-in-game">
              ...
              <div className="room-code-box-in-game">
                <span className="room-code-text-in-game">{currentMatch.room_code}</span>
                <button
                  type="button"
                  onClick={() => {
      ...
      {showGameOverModal && gameOverMessage && (
        <div className="game-over-modal-overlay" onClick={() => setShowGameOverModal(false)}>
                ...
                <div className="game-over-elo-info">
                  ...
                  <div className="elo-info-content">
                    ...
                    <div className="elo-after">
                      ...
                      <span className={`elo-value ${eloChange > 0 ? 'elo-positive' : eloChange < 0 ? 'elo-negative' : ''}`}>
                        {finalElo.after}
                      </span>
                    </div>
                    <div className="elo-change">
                      <span className={`elo-change-value ${eloChange > 0 ? 'elo-positive' : eloChange < 0 ? 'elo-negative' : ''}`}>
      ...
      {showPlayerColorModal && playerColor && (
        <div className="player-color-modal-overlay" onClick={() => setShowPlayerColorModal(false)}>
          <div className="player-color-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="player-color-modal-content">
              <div className={`player-color-badge ${playerColor === 'B' ? 'badge-black' : 'badge-white'}`}>
                {playerColor === 'B' ? '⚫' : '⚪'}
