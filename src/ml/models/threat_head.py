"""
Threat detection head for multi-task model.

Outputs a heatmap indicating threat levels across the board.
"""
...
class ThreatHead(nn.Module):
    """
    Head for threat detection task.

    Outputs a heatmap of shape (batch, board_size, board_size)
    where values range from 0 (no threat) to 1 (high threat).
    """
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Threat heatmap of shape (batch, board_size, board_size)
...
def test_threat_head():
    """Test the threat head with dummy data."""
    input_channels = 64
    board_size = 9
    batch_size = 2

    head = ThreatHead(input_channels=input_channels, board_size=board_size)
    ...
    print(f"Output threat map shape: {output.shape}")
