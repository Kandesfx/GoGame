...
def extract_year_from_filename(filename):
    """Extract year from filename format: YYYY-MM-DD-XX.sgf"""
    match = re.match(r'(\d{4})-\d{2}-\d{2}-\d+\.sgf', filename)
    if match:
...
def process_sgf_files_parallel(sgf_files, num_workers=None):
    ...
    with Pool(processes=num_workers) as pool:
        results = list(tqdm(
            pool.imap(parse_single_sgf_file, sgf_files),
            total=len(sgf_files),
            desc="Parsing SGF files"
        ))
