...
def upgrade() -> None:
    # Add time control columns to matches table
    op.execute("""
        DO $$ 
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM information_schema.columns 
                WHERE table_name = 'matches' AND column_name = 'time_control_minutes'
            ) THEN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM information_schema.columns 
                WHERE table_name = 'matches' AND column_name = 'black_time_remaining_seconds'
            ) THEN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM information_schema.columns 
                WHERE table_name = 'matches' AND column_name = 'white_time_remaining_seconds'
            ) THEN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM information_schema.columns 
                WHERE table_name = 'matches' AND column_name = 'last_move_at'
            ) THEN
...
def downgrade() -> None:
    # Remove time control columns from matches table
    op.execute("""
        DO $$ 
        BEGIN
            IF EXISTS (
